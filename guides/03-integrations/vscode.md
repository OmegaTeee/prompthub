...
#### Issue: "Claude Code Shows Disconnected Status"

**Symptoms:**
- Claude Code sidebar shows "Disconnected" status
- No MCP tools available in extension
...
4. **Verify extension is latest version:**
   - Check for Claude Code / Cline extension updates
...
1. **Verify Qwen3-Coder model is installed:**
   ```bash
...
- Cannot read/write files outside workspace
- macOS blocks file access

**Solutions:**

1. **Grant VS Code Full Disk Access:**
   - System Settings → Privacy & Security → Full Disk Access
   - Click `+` → Add "Visual Studio Code"
   - Restart VS Code
...
3. **Verify desktop-commander server is running:**
   ```bash
   curl http://localhost:9090/servers | jq '.[] | select(.name=="desktop-commander")'
