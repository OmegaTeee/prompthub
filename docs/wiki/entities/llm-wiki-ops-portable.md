...
A Claude Code skill implementing the Karpathy LLM Wiki pattern for any workspace. Installed user-level at `~/.claude/skills/llm-wiki-ops-portable/SKILL.md` and auto-discovered. Powers PromptHub's wiki at `docs/wiki/` (see [[../concepts/llm-wiki-setup]] for project-specific framing).
...
Trigger phrases: invocation via `/skill llm-wiki-ops-portable` or natural-language hints like "add this to the wiki" / "make a wiki page for X" / "search the wiki for Y". The project also exposes thin slash-command wrappers (`/wiki-add`, `/wiki-find`, `/wiki-lint`, `/wiki-log`) that delegate to the skill — see [[../concepts/llm-wiki-setup]].

## Related

- [[../concepts/llm-wiki-setup]] — how this skill is used in PromptHub specifically
