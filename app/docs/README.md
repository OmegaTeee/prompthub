# Developer Documentation

Technical documentation for PromptHub developers and maintainers.

## Documentation Structure

### Audit System
Comprehensive audit logging and security monitoring implementation.

- **[audit/AUDIT-IMPLEMENTATION-COMPLETE.md](audit/AUDIT-IMPLEMENTATION-COMPLETE.md)** - Executive summary of all 3 audit phases
- **[audit/AUDIT-CODE-REVIEW.md](audit/AUDIT-CODE-REVIEW.md)** - Initial code review and recommendations
- **[audit/PHASE-1-AUDIT-IMPLEMENTATION.md](audit/PHASE-1-AUDIT-IMPLEMENTATION.md)** - Structured logging and context propagation
- **[audit/PHASE-3-COMPLIANCE-MONITORING.md](audit/PHASE-3-COMPLIANCE-MONITORING.md)** - Integrity verification, security alerts, SIEM integration

**Quick Links:**
- Security Score: 3.2/10 → 9.0/10
- Compliance: SOC 2 (90%), GDPR (85%), HIPAA (80%)
- API Reference: See PHASE-3-COMPLIANCE-MONITORING.md

## Other Documentation

### Build & Development
- [BUILD-SPEC.md](../BUILD-SPEC.md) - Build specifications and requirements
- [BUILD-TASKS.md](../BUILD-TASKS.md) - Development task tracking
- [CLAUDE.md](../CLAUDE.md) - Claude Code integration guide

### Implementation Guides
- [KEYRING-INTEGRATION-COMPLETE.md](features/KEYRING-INTEGRATION-COMPLETE.md) - Keyring credential management
- [DASHBOARD-IMPROVEMENTS.md](dashboard/DASHBOARD-IMPROVEMENTS.md) - Dashboard enhancement documentation
- [SECURITY-FIXES.md](security/SECURITY-FIXES.md) - Security improvements and fixes

### User Guides
User guides (setup, configuration, integrations) live in the Obsidian vault at `~/Vault/PromptHub/`.

## Contributing

For development setup and contribution guidelines, see the main [README.md](../README.md).
