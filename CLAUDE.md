...
- **Model roles**: The router daemon defaults to `Qwopus3.5-4B-v3-GGUF` (distilled-Opus 4B) for enhancement and `qwen3-4b-thinking-2507` for the orchestrator (see ADR-008). Per-client `model_profile` in `enhancement-rules.json` opts specific clients into alternate models — `coder` → `Qwopus3.5-9B-Coder-GGUF` (used by `vscode`/`claude-code`), `claude_feel` → 27B distilled planner, `instruct` → `qwen3-4b-instruct-2507` (vanilla fallback). The `settings.py` `llm_model` default stays at vanilla `qwen3-4b-instruct-2507` as the code-level safety net.
...
| **On-demand servers need manual start** | Servers with `auto_start: false` (`chrome-devtools-mcp`) are not running by default. Start them via `curl -X POST http://localhost:9090/servers/{name}/start` using desktop-commander, then wait for the bridge to refresh its tool list. |
...
| **Comet tools require Comet browser** | `comet_*` tools (aliased from `perplexity-comet`) need the Comet browser running. The bridge auto-starts Comet via `comet_connect`, but initial connection may take 5-10 seconds. |
