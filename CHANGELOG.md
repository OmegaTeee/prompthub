...
- **Progressive tool disclosure (bridge meta-tools)**: Added `discover_tools` (lightweight catalog) and `load_server_tools` (on-demand server tool promotion) to `mcps/prompthub-bridge.js`, gated by `TOOL_DISCLOSURE`/`TIER1_SERVERS` and emitting `notifications/tools/list_changed` after loads.
- **Client profile endpoints (read-only)**: Added `GET /clients/{name}/tool-profile` and `GET /clients/{name}/model-profile` derived from `configs/enhancement-rules.json` (separate `tool_profile` and `model_profile` blocks; can be combined later).
- **Model download helper**: Added `scripts/models/download-qwen-distilled.sh` + `scripts/models/qwen-distilled.env.example` to reproducibly download the coder-first distilled GGUF set via `huggingface-cli download`.
...
### Changed
- **Coder-first default model profile**: Switched router/enhancement defaults from `qwen3-4b-instruct-2507` to `Qwopus3.5-4B-v3-GGUF` and introduced `model_profiles` + per-client `model_profile` resolution in enhancement rules.
- **Orchestrator model alignment**: Switched `LLM_ORCHESTRATOR_MODEL` default and orchestrator agent `DEFAULT_MODEL` from `qwen3-4b-thinking-2507` to `Qwopus3.5-4B-v3-GGUF` and appended `/no_think` to the orchestrator system prompt to avoid visible thinking output.
