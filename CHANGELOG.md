...
### Added
- **Launchctl token environment audit**: Added `scripts/system/launchctl-env-audit.sh` to compare GUI-session `launchctl` token variables against Keychain-backed values without printing secrets, sync known token env vars when needed, and unset stale launchd values explicitly.
