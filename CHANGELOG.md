...
### Changed
- **Qwen Code: unified single `settings.json` replacing the direct/router toggle pair** (this PR): `clients/qwen-code/settings.json` lists every provider with **`name` set to match `id`** so that clients which ambiguously send either field (notably the VS Code Companion, which sends `name` as the model parameter) always hit a valid model id at LM Studio or OpenRouter. Human-readable labels live in the `description` field — they're never sent in API requests. The 8 providers: PromptHub Router as default (`qwen3-4b-instruct-2507` and `-thinking-2507` on `:9090`), LM Studio Direct for models the router doesn't proxy (`qwen3-0.6b` fast model, `qwen/qwen3-vl-4b` vision, Qwen2.5 Coder 3B/7B/14B on `:1234`), and OpenRouter Free Router (`openrouter/free`) as cloud fallback. The pre-D draft included LM Studio Direct fallbacks for `qwen3-4b-instruct-2507` and `-thinking-2507` (same ids as the Router entries on different baseUrls) — removed because with `name = id`, the `/model` picker would show two visually indistinguishable entries. PromptHub Router already does cloud fallback via OpenRouter for free_ok/any clients, so the LM Studio Direct duplication wasn't earning its keep. Provider switching happens at runtime via Qwen Code's `/model` picker. Deleted `settings.direct.json` and `settings.router.json`. Simplified `setup.sh` (no mode arg, copy-with-backup instead of symlink) and `check.sh` (no mode logic, drift detection vs repo). PromptHub Router entries send both `X-Client-Name` and `X-Client-ID` headers, pre-empting the audit-context trap surfaced in PR #23. Personal dotfile follow-up (since `clients/dotfiles/shell_common.sh` is a symlink to a user file, not tracked content): replace `qwen-code-direct` / `qwen-code-router` aliases with `qwen-code-setup` and `qwen-code-check` — see README.
- **LM Studio: `unloadPreviousJITModelOnLoad: false`** in `clients/lm-studio/settings.json` — keep previously-loaded models warm when switching, so the unified Qwen Code config (which lists the same model under multiple providers) doesn't thrash model loads. Combined with JIT and `jitModelTTL.ttlSeconds: 3600`, models stay in memory for an hour after last use.
