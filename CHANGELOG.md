...
### Changed
- **Qwen Code: unified single `settings.json` replacing the direct/router toggle pair** (this PR): `clients/qwen-code/settings.json` now lists every provider — PromptHub Router as default (`qwen3-4b-instruct-2507` and `-thinking-2507`), Direct LM Studio as fallback (same Qwen3 ids on `:1234`, plus Qwen3 0.6B fast model, Qwen3 VL vision, and Qwen2.5 Coder 3B/7B/14B), and OpenRouter Free Router as cloud fallback. Per Qwen Code's docs, providers are uniquely keyed by `id` + `baseUrl`, so the same model id appears twice (once via Router, once Direct) without collision; provider order encodes the default (`model.name = "qwen3-4b-instruct-2507"` resolves to the first match → PromptHub Router). Provider switching happens at runtime via Qwen Code's `/model` picker. Deleted `settings.direct.json` and `settings.router.json`. Simplified `setup.sh` (no mode arg) and `check.sh` (no mode logic). PromptHub Router entries send both `X-Client-Name` and `X-Client-ID` headers, pre-empting the audit-context trap surfaced in PR #23. Personal dotfile follow-up (since `clients/dotfiles/shell_common.sh` is a symlink to a user file, not tracked content): replace `qwen-code-direct` / `qwen-code-router` aliases with `qwen-code-setup` and `qwen-code-check` — see README.
