...
### Added
- **Doc-drift gate via Qwen3** (this PR): New `scripts/doc-drift/check-doc-drift.sh` runs as a `.githooks/pre-commit` hook, a Qwen Code stop hook (`scripts/doc-drift/qwen-stop-hook.sh` wired via project-local `.qwen/settings.json` so it activates automatically per checkout — no `~/.qwen/settings.json` edits required), and a GitHub Actions workflow (`.github/workflows/doc-drift.yml`). Deterministic gate (`CODE_PATTERNS` / `DOC_PATTERNS` / `IGNORE_PATTERNS`) blocks commits that change runtime code without an accompanying doc update; optional second-pass review against LM Studio (default `qwen3-4b-instruct-2507`) blocks the commit when the model returns `"fail"` in its JSON verdict; connection errors and timeouts (15 s cap) silently pass so a wedged or absent LM Studio can't hold up commits. Fixes vs. the initial draft: dropped `.claude/` from `CODE_PATTERNS` (per-developer config); exempted `app/tests/`, any `*/tests/`, `*/test_*.py`, and `scripts/doc-drift/` itself; replaced the broken commit-message escape hatch (which checked the *previous* commit in pre-commit mode) with `DOC_DRIFT_SKIP=1` env var and CI-only `[doc-drift:ignore]`; resolved `BASE_REF` correctly in GH Actions (`origin/${GITHUB_BASE_REF}`); added `curl --max-time 15` so a wedged LM Studio can't block commits; derived `REPO_ROOT` in the Qwen hook from `BASH_SOURCE` so it works for any checkout. Setup doc at `docs/notes/qwen-doc-drift-setup.md`.
