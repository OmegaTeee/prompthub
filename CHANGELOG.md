...
- **Progressive tool disclosure — bridge meta-tools (this PR)**: Added `discover_tools` and `load_server_tools` to `mcps/prompthub-bridge.js`, gated by two env vars: `TOOL_DISCLOSURE=full|progressive` (default `full`, preserves existing behavior) and `TIER1_SERVERS=a,b,c` (servers seeded into the active set on startup). In `progressive` mode, `tools/list` returns only tier-1 + explicitly loaded servers + meta-tools, cutting initial tool-context by ~80% on typical fleets. `discover_tools` returns a schemas-free catalog; `load_server_tools` promotes a server and emits `notifications/tools/list_changed` so the client refreshes. Phase 1 is env-driven; per-client routing of disclosure mode lands in a follow-up PR via `GET /clients/{name}/tool-profile`.
