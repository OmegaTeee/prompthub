...
### Added
- **Goose ACP provider baseline**: Updated `clients/goose/config.yaml` to keep `lmstudio` active by default while adding selectable `codex-acp`, `pi-acp`, and `claude-acp` providers plus a repo-managed Top Of Mind policy file that routes Obsidian note work through `obsidian-mcp-tools_*` instead of raw shell writes.
- **Launchctl token environment audit**: Added `scripts/system/launchctl-env-audit.sh` and `docs/guides/12-launchctl-token-env-guide.md` to compare GUI-session `launchctl` token variables against Keychain-backed values without printing secrets, sync known token env vars when needed, and unset stale launchd values explicitly.
- **`GET /v1/health` endpoint** (PR #29): Lightweight OpenAI-style health probe under the `/v1` prefix for clients that follow the LM Studio convention of pre-flight-checking `/v1/health` before issuing requests. The existing root `/health` route remains the canonical detailed status endpoint with the full supervisor snapshot, services map, and cache statistics; `/v1/health` returns a minimal `{"status": "healthy", "servers": {"total", "running", "failed"}}` payload so probes get a 200 without paying the cost of the full snapshot. Falls back to just `{"status": "healthy"}` during early startup before the supervisor is initialized.
- **Codex router tool profile**: Added a `codex` `tool_profile` in `app/configs/enhancement-rules.json` so bridge sessions can use progressive disclosure with `desktop-commander`, `context7`, and `memory` as tier-1 servers instead of falling back to the router default `full` mode.
- **Headroom MCP server removal**: Dropped the extra `headroom mcp serve` registration from `clients/codex/config.toml` and `clients/vscode/mcp.json`; both clients still use Headroom through the shared `8787` proxy path without spawning separate stdio MCP processes.
- **Headroom CLI helper wrappers**: Added `hdiff`, `hloc`, and `hsg` in `clients/dotfiles/shell_common.sh` for Headroom-backed difftastic, scc, and ast-grep usage; noted MCP prompts/resources as the next follow-on in `TODOS.md`.
- **Progressive tool disclosure — bridge meta-tools** (PR #31): Added `discover_tools` and `load_server_tools` to `mcps/prompthub-bridge.js`, gated by two env vars: `TOOL_DISCLOSURE=full|progressive` (default `full`, preserves existing behavior) and `TIER1_SERVERS=a,b,c` (servers seeded into the active set on startup). In `progressive` mode, `tools/list` returns only tier-1 + explicitly loaded servers + meta-tools, cutting initial tool-context by ~80% on typical fleets. `discover_tools` returns a schemas-free catalog; `load_server_tools` promotes a server and emits `notifications/tools/list_changed` so the client refreshes.
- **Progressive tool disclosure — router profile API** (PR #32): New read-only endpoint `GET /clients/{name}/tool-profile` returns the resolved `{disclosure, tier1_servers, source}` for a client, derived from `app/configs/enhancement-rules.json`. The bridge fetches this at startup when `TOOL_DISCLOSURE` and `TIER1_SERVERS` are both unset, so disclosure config can be centralized in the router instead of duplicated per-client launch script. Precedence is explicit env > router profile > default `full`. Dashboard's Token Budget panel adds a "Tools" column showing each client's disclosure mode and tier-1 list. Opt-in clients seeded in config: `claude-desktop`, `claude-code`, `vscode`, `open-webui`.
- **Model profiles — opt-in mechanism for testing new model families** (PR #33): Added a top-level `model_profiles` block in `enhancement-rules.json` mapping a short name (`daemon`, `coder`, `claude_feel`) to a model id, plus a per-client `model_profile` key that resolves to the profile's model at enhancement time. Daemon defaults are unchanged (`qwen3-4b-instruct-2507`) so every existing client behaves identically; only clients that opt in see a different model. Seeded opt-ins: `vscode` and `claude-code` → `coder` (`Qwopus3.5-9B-Coder-GGUF`). New read-only endpoint `GET /clients/{name}/model-profile` returns `{model_profile, resolved_model, source}` with a distinct `profile_missing` source value when a client references an unregistered profile name (catches typos at config-edit time). New distilled models registered in `MODEL_CONTEXT_TOKENS` at 262K window: `Qwopus3.5-9B-Coder-GGUF`, `Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled-v2-GGUF`. Dashboard's Token Budget panel surfaces the profile name and resolved model side-by-side.
- **Model downloader script** (PR #34): Added `scripts/models/download-qwen-distilled.sh` and `scripts/models/qwen-distilled.env.example` for reproducibly fetching the coder-first distilled GGUF set (`Qwopus3.5-4B-v3-GGUF`, `Qwopus3.5-9B-Coder-GGUF`, `Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled-v2-GGUF`) via `huggingface-cli download`. Profile names (`daemon`, `coder`, `claude_feel`) match the `model_profiles` block in `enhancement-rules.json`. All repos and glob patterns are env-var-driven; the `.env.example` is the single swap point for deployment-time changes. Auth prefers `PH_HF_TOKEN`, falls back to `HF_TOKEN` then `HUGGINGFACE_API_KEY`. `--dry-run` prints the exact `huggingface-cli` invocations without running them and never echoes tokens. ShellCheck-clean.

### Changed
- **Headroom generated Codex config synced**: Added the PromptHub bridge stanza to `headroom/.codex/config.toml` so `headroom init` output still launches `prompthub-bridge` with the repo-local path.
- **Headroom wrapper cwd preservation**: Removed the extra `cd` from the Headroom shell helpers in `clients/dotfiles/shell_common.sh` so `headroom wrap codex` and `headroom wrap claude` launch from the caller's current directory.
- **Codex bridge path cleanup**: Replaced the `~/prompthub` working directory in `clients/codex/config.toml` and `clients/codex/codex-config.toml` with an explicit repo path so `prompthub-bridge` starts reliably under Codex.
- **Headroom OpenAI helper**: Added `headroom_openai()` to `clients/dotfiles/shell_common.sh` as a generic opt-in wrapper for ad hoc OpenAI-compatible calls through Headroom without changing the shared shell default.
- **Headroom shell default narrowed**: Restored the shared `OPENAI_BASE_URL` in `clients/dotfiles/shell_common.sh` to PromptHub on `9090` so only the Headroom wrappers route through `8787`.
- **Headroom shell helper tightening**: Corrected the proxy upstream token in `clients/dotfiles/shell_common.sh` to use `LM_API_TOKEN` and changed `headroom_proxy()` to restart the LaunchAgent instead of spawning a competing proxy process.
- **Headroom persistent startup**: Added `scripts/launch-agents/com.prompthub.headroom.plist` and hardened `headroom/launch.sh` so the proxy can be installed as a LaunchAgent and survive login sessions.
...
- **Dotfile GitHub token aliases**: Exported `GITHUB_PAT_TOKEN` from the Keychain-backed `GITHUB_PAT` value so Codex MCP tools can use the same token source as existing GitHub shell aliases.
- **Codex client config refreshed**: Updated `clients/codex/config.toml` for current Codex defaults (`gpt-5.5`), bundled browser/node REPL support, Sites plugin enablement, and desktop context-detail settings used by the live `~/.codex/config.toml` symlink.
...
### Removed
- **Unused `browsermcp` and `applescript-mcp` servers**: Removed both MCP entries from `app/configs/mcp-servers.json`, dropped their npm dependencies from `mcps/package.json`, and updated current docs/prompts to reflect `chrome-devtools-mcp` as the only remaining on-demand router-managed server.
...
### Docs
- **Documentation hygiene for PRs #31–#33** (PR #35): Updated `CLAUDE.md` Key Patterns to point at `model_profile` / `tool_profile` rather than implying a single fixed model per client; added the two new `/clients/{name}/*-profile` endpoints to the API list; expanded the `enhancement-rules.json` description to mention `model_profiles` / `model_profile` / `tool_profile`. Added a "2026-05-25 — Per-Client Model Profiles (Opt-In)" update block to ADR-008 explaining the opt-in mechanism without superseding the canonical two-model architecture. Glossary gains `Model profile` and `Tool profile` entries and updates `Progressive disclosure` from `*(Planned)*` to shipped state. `TODOS.md` ticks Phase 2.
- **Copilot-review cleanup pass (this PR)**: Corrected the docstring claim that `discover_tools` POSTs to `/mcp/{server}/tools/list` (it actually goes through `/tools/call` with JSON-RPC `method: "tools/list"`) in `mcps/prompthub-bridge.js` and `docs/notes/plans/progressive-tool-disclosure.md`. Tightened the `model_profile` rendering in the dashboard Token Budget panel so a typo'd profile name shows a "not found" badge instead of misleadingly reading "via profile X" alongside the fallback model — added `model_profile_resolved` boolean from `dashboard/router.py` to the template. Sharpened the partial-env-config wording in `docs/glossary.md`, `docs/guides/11-progressive-tool-disclosure.md`, and `mcps/README.md` to specify that the router fetch fires only when *both* `TOOL_DISCLOSURE` and `TIER1_SERVERS` are unset, and documented the two failure modes when only one is set. Made the meta-tools-always-present fact explicit in the user guide. Stripped manual column alignment in `MODEL_CONTEXT_TOKENS` so `ruff format` is stable. Replaced stale "(this PR)" references in earlier changelog entries with their canonical PR numbers.
- **Progressive Tool Disclosure user guide** (PR #36): Added `docs/guides/11-progressive-tool-disclosure.md` — an end-user guide covering what disclosure is (kitchen-drawer analogy + before/after), how to enable it per client (`tool_profile` block or `TOOL_DISCLOSURE`/`TIER1_SERVERS` env vars), how the agent discovers and loads tools on demand, tier-1 selection guidance, and a Testing & Verification section with five chat examples (including a false-pass warning). Added a "Progressive tool disclosure" component-reference section to `mcps/README.md` (env vars, the `discover_tools`/`load_server_tools` meta-tools, env > router > full precedence, active-set rules, startup log). Indexed the new guide in `docs/guides/README.md`.

### Fixed
- **MCP bridge structured tool results**: Preserved proxied `tools/call` results that already include MCP `content` blocks, including `structuredContent` for tools with `outputSchema` such as `sequential-thinking`. This prevents strict MCP clients from rejecting bridged tools with "has an output schema but no structured content was provided."
- **Two `500`s on GET session routes surfaced by schemathesis fuzzing** (PR #51): `GET /sessions/{id}/facts?tags=…` and `GET /sessions` returned unhandled `500`s instead of the documented `200`/`422`. Both were unvalidated request values reaching SQLite unguarded. (1) The fact tag-filter built SQL with `json_extract(tags, '$[*]')`, but `$[*]` is not valid `json_extract` path syntax — SQLite raised `OperationalError: bad JSON path: '$[*]'` for *any* non-empty `tags` value (the fuzzer's `tags=null` parses to the truthy list `["null"]`). Rewrote the filter with `json_each` + `EXISTS (… WHERE value IN (…))` so it matches exact tags and unknown tags return `200 []`. (2) `list_sessions` passed `offset`/`limit` straight into a `LIMIT ? OFFSET ?` bind, so a value above SQLite's signed-64-bit max (e.g. `offset=270164148743501611008`) raised `OverflowError`; `list_sessions` and `get_facts` now clamp pagination via a `_clamp_pagination` helper (mirroring the existing `search()` clamp). Verified by re-running the GET-only `app/schemathesis.toml` pass — Server-error count dropped 1 → 0.
- **OpenAPI schema now documents `404` on GET-by-resource routes** (PR #51): seven GET routes return `404` for unknown resources but previously declared only `200`/`422`, so the schema misrepresented reality to clients and fuzzers. Added `responses={404: …}` to `GET /sessions/{id}`, `/sessions/{id}/facts`, `/sessions/{id}/memory/{key}`, `/sessions/{id}/context`, `/tools/{server}`, `/health/{server}`, and `/servers/{name}`.
- **`api-keys.json` shape regression silently disabled the OpenAI-compat proxy**: The live `app/configs/api-keys.json` had been rewritten as a bare array (`{"keys": [ {...} ]}`), but `ApiKeyManager` loads into `ApiKeysRegistry` whose `keys` field is a `dict[str, ApiKeyConfig]` map keyed by the bearer token. The array failed Pydantic validation; `load()` caught the error, logged it, and left the registry empty — so the fail-closed loader rejected **every** `/v1/` request with 401 and no obvious cause. Restored the token-keyed map shape with a reconciled 10-client roster (`claude-code`, `claude-desktop`, `codex`, `vscode`, `open-webui`, `perplexity`, `lm-studio`, `comfyui`, `cherry-studio`, `default`); dropped unused `cursor`, `raycast`, `opencode`. Each client keyed by its `sk-prompthub-<client>-001` token. Corrected the `open-webui` `client_name` spelling (was `openwebui` in a prior backup), which must match the `enhancement-rules.json` lookup key or the client silently falls back to default enhancement.
- **`api-keys.json.example` had drifted to an unloadable shape**: The example was a top-level array with a bogus `api_key` *field* (no such field exists on `ApiKeyConfig`) — copying it to `api-keys.json` reproduced the fail-closed bug above, making it the likely origin of the regression. Regenerated as a correct token-keyed map with placeholder `open-webui` + `default` entries. Added `TestShippedConfigArtifacts` to `tests/test_openai_compat.py` that loads both `api-keys.json` and `api-keys.json.example` through the real `ApiKeyManager` and asserts a non-empty, well-formed registry — so neither file can silently drift from the loader again. Also corrected the stale `sk-prompthub-openwebui-001` → `sk-prompthub-open-webui-001` token across all references so users following the guides/scripts don't hit 401s: `clients/_open-webui/setup.sh`, `scripts/_open-webui/start.sh`, `docs/guides/09-open-webui-guide.md`, `docs/guides/06-client-configuration-guide.md`, and the unreleased Open WebUI integration changelog entry.
- **`/v1/chat/completions` and `/v1/responses` model-name guard rejected legitimate names with spaces** (PR #28): the placeholder-rejection guard (`if " " in body.model`) was meant to catch OpenAPI swagger defaults like `"string"` and `"model"`, but it also rejected real provider display names like `"PromptHub Router · Qwen3 4B Instruct 2507"` or `"GPT-4 Turbo (via OpenRouter)"` that some clients (notably Qwen Code's VS Code Companion) send. Surfaced as `Internal error: 422 status code (no body)` because the error body was a plain `{"detail": "string"}` instead of the OpenAI-standard `{"error": {"message": ..., "type": ...}}` envelope that SDK clients parse. Fix: drop the space check (placeholder strings `"string"` / `"model"` / empty are still rejected) and reshape the 422 body to OpenAI's error format with `type`, `param`, and `code` fields. Same guard appears in both endpoints; both updated.
...
### Fixed
- **Daemon-safe Keychain account resolution** (PR #19): Replaced `os.getenv("USER")` with `getpass.getuser()` in `KeyringManager`, `Settings._get_from_keyring`, and `manage-keys.py`. `getpass.getuser()` falls through to `pwd.getpwuid(os.getuid())` when `USER` is unset (LaunchAgent context), preventing fallback to literal `"default"` account and unreachable credentials.
- **qwen-code config bugs surfaced by review** (PR #19): `check.sh` `DIRECT_FILE`/`ROUTER_FILE` aligned with `setup.sh` filenames (was reporting `unknown` mode); `settings.direct.json` env `LM_API_TOKEN` sourced from `${LM_API_TOKEN}` (was `${LMSTUDIO_API_KEY}`); `envKey` values switched from interpolation form to bare variable names that Qwen Code expects; `settings.router.json` `PROMPTHUB_API_KEY` references `${PROMPTHUB_QWEN_CODE_TOKEN}` (was hardcoded literal).
- **qwen-code config alignment** (PR #18): `setup.sh` filename references fixed (`settings.direct-lmstudio.json` → `settings.direct.json`, `settings.prompthub-router.json` → `settings.router.json`); `mcpServers` blocks separated from `settings.*.json` into standalone `mcp.json` matching Qwen Code's preferred layout; literal LM Studio token in `settings.router.json` replaced with `${LM_API_TOKEN}` env var reference.
- **Ruff debt cleared, lint CI strictened** (PR #16): Fixed 10 `router/` Ruff errors (E402/F841/F541/I001). Dropped `continue-on-error: true` from `ci.yml`'s Lint job, removed duplicate Ruff/mypy sub-steps from `tests.yml`, and deleted the `claude-code-review` auto-runner workflow.
- **Hygiene audit sources and client docs**: Updated the hygiene skill for the current Codex and client layout, removed retired `openclaw` and Copilot API keys, switched VS Code proxy examples to `sk-prompthub-vscode-001`, documented `.vscode/settings.json` and `clients/vscode/global-settings.json` as the active VS Code settings sources, and normalized Cherry Studio metadata and docs to `cherry-studio` including the live PromptHub token name.
...
### Added
- **Cross-session memory search**: New `POST /sessions/search` endpoint and `prompthub_memory_search` bridge meta-tool. SQLite FTS5-backed BM25 ranking over `session_facts` + `session_memory_blocks`; client_id-scoped by default with `cross_client: true` opt-in. Implements Tier 2 of the comet research-delegate plan — chat models now have a "thread context first" path before reaching for browser tools. Updates `clients/TOOL_USE.md` with a memory-search-first routing rule and a "Memory search before research" section.
- **Agent-initiated server start meta-tools** (PR #20): The bridge now exposes two synthetic tools for managing on-demand servers — `prompthub_list_available_servers` (returns all configured servers with status) and `prompthub_start_server` (POSTs to `/servers/{name}/start`, polls for `running` status, then sends a `notifications/tools/list_changed` so MCP clients re-fetch tools). Lets agents discover and start `auto_start: false` servers (`obsidian`, `chrome-devtools-mcp`, `browsermcp`, `applescript-mcp`, `homebrew`) without out-of-band intervention. Prerequisite for Progressive Tool Disclosure Phase 1.
- **`llm_api_key` keyring fallback** (PR #15): `Settings.model_post_init` now resolves `llm_api_key` from Keychain account `lm_api_token` (service `prompthub`) when not set in env, mirroring the existing `openrouter_api_key` resolution. Added `LM_API_TOKEN` env-var alias (LM Studio's official name).
- **`applescript-mcp` + `homebrew` MCP servers** (PR #18): `@peakmojo/applescript-mcp` and `brew mcp-server` registered with `auto_start: false`. Direct sidecar entries (`applescript-mcp-server`, `fetch`, `homebrew`, `applescript_mcp`) stripped from `qwen-code`, `claude`, and `vscode` client configs (now route through prompthub-bridge for circuit-breaker, audit, and enhancement). Server count: 9 → 11.
- **`manage-keys.py list --all` mode** (PR #18): Enumerates Keychain entries directly via `security dump-keychain`, surfacing orphaned (set but unreferenced) and legacy (`svce='prompthub'`, pre-migration) entries beyond the static discovery sources in `mcp-servers.json` + `SETTINGS_KEYS`.
- **Per-server `proxy_timeout` override**: New optional `ServerConfig.proxy_timeout` field (seconds, float) lets `mcp-servers.json` raise the bridge dispatch ceiling above the proxy default (`DEFAULT_BRIDGE_TIMEOUT = 30.0` in `app/router/routes/mcp_proxy.py`). Response `metadata.timeout_used` echoes the resolved value for dashboard/debug visibility. Set `"proxy_timeout": 180` on `perplexity-comet` so `comet_ask` (120 s default) no longer trips the circuit breaker mid-flight when agentic browsing exceeds 30 s. Other servers continue to use the 30 s default.
...
### Changed
- **Comet tool prefix renamed `perplexity_*` → `comet_*`**: Bridge `TOOL_PREFIX_ALIASES` for the `perplexity-comet` server now uses `displayPrefix: 'comet'` (was `'perplexity'`). The tools control a Comet browser via CDP; "comet" reflects what the tools do, while "perplexity" was leaning on the parent brand. Affected names: `comet_ask`, `comet_connect`, `comet_poll`, `comet_screenshot`, `comet_stop`, `comet_tabs`, `comet_mode`, `comet_upload`. The `perplexity` enhancement rule's system prompt was simultaneously tuned for Comet's UI behavior (complete-sentence questions, explicit URLs, mode hints), and `clients/TOOL_USE.md` plus the active LM Studio thinking preset were updated. The PR #10 alias scheme is unchanged otherwise; only the display prefix swapped.
- **Unified Keychain naming on `prompthub:<key>/$USER`** (PR #18): PromptHub credentials migrated from `service=prompthub, account=<key>` to `service=prompthub:<key>, account=$USER`. `KeyringManager`, `Settings._get_from_keyring`, and `manage-keys.py` build service names from a `prompthub:` prefix; `mcp-servers.json` `{source: "keyring"}` entries dropped the now-redundant `service` field. Each credential now appears as a distinct entry in Keychain Access. 12 entries migrated; 8 legacy duplicates and 5 orphan duplicates cleaned up post-verification. Shell helper `keychain_secret()` in `clients/dotfiles/shell_common.sh` extended to a 3-level fallback chain (`prompthub:<key>/$USER` → `prompthub/<key>` → `<KEY-UPPERCASE>/$USER`).
- **`manage-keys.py` moved to `app/scripts/`** (PR #18): Canonical invocation is now `python scripts/manage-keys.py` from `app/` with venv active. Empty `scripts/security/` directory removed. README, `app/.env.example`, `app/configs/README.md`, `mcps/README.md`, and `scripts/README.md` updated to match.
- **`mcps/README.md` accuracy** (PR #19): Server roster updated 10 → 11 (added `applescript-mcp` + `homebrew`, fixed `obsidian-mcp-tools` entry that was previously listed by command name `mcp-obsidian-vault`). Replaced `~/`-prefixed paths in markdown links (which GitHub doesn't expand) with repo-relative links and inline code spans. Keyring example dropped redundant `service` field.
...
- **Open WebUI integration**: First-class PromptHub client connecting via HTTP (`/v1/` proxy + `/mcp-direct/mcp` Streamable HTTP). API key `sk-prompthub-open-webui-001` with enhancement disabled for lower latency. CLI support (`generate`, `install`, `validate`, `list`), `GET /configs/open-webui` endpoint, dashboard panel with connection status (15s HTMX polling), launch scripts (`start.sh`, `stop.sh`), and macOS LaunchAgent plist. 13 new tests (8 unit, 5 integration).
